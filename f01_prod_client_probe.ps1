...
# Requires a test client account provisioned via the admin Clients page first.
...
function Get-Rows($table, $qs = "") {
    ...
    try {
        ...
        return ,$r   # comma forces array even for single item
    } catch { return @() }
}
...
# profiles — expect exactly 1 row, role=client
...
# get_client_project_summary RPC
try {
    ...
    if ($sCount -eq 0) {
        ...
        ok "get_client_project_summary → $sCount project(s) (verify names are own company)"
