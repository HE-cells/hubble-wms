...
> *(Revised: 2026-06-30 — 🔎 **Round 49 recheck + pre-launch audit plan.** Created [PRE_LAUNCH_AUDIT_PLAN.md](PRE_LAUNCH_AUDIT_PLAN.md) (5 phases; order: must-fix CONV-M4+L-CSP → audit Phases 1–4 → team review → roster swap). **Branch hygiene:** 6 stale branches confirmed safe to delete (`pre-launch-audit`, `f01-probe`, `employee-modal-copy-button-eddm9j`, `help-page-layout-kglm8b`, `current-baseline-trp6qo`, `review-9hfmhc`) — all squash-merged in R40–R45; main is far ahead; "potential loss" line-counts were diff artifacts (main holds newer versions). **⚠️ FINDING: F-05 is code-DONE (R44) — audit plan corrected (was wrongly listed as "build before launch"). BUT `20260629_request_review_rpcs.sql` apply in prod is UNCONFIRMED** (not in CLAUDE.md applied-list; R44 note says "needs Studio apply") while main's `requests.js`/`jobTitleRequests.js` already call those 3 RPCs → if unapplied, approving deletion/name-change/job-title requests fails live. **Added Phase 1H** to verify (`SELECT proname FROM pg_proc WHERE proname IN (...)` → expect 3). No code/cache change (v=113).)*
