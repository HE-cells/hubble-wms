...
> **⏱ UPDATED 2026-06-17 (R39) — supersedes the blocks below.** Login overhaul LIVE; R25 RLS sweep applied+verified. **Prod serves JS v=98 / CSS v=34** (R39 CLIENT-01 LIVE `c0e7fdc` + search-icon fix `6690861`; R38 `8f732e0` v=96). **✅ Entire Medium-severity audit queue CLOSED (R38)** — M-DATE, M-SETTLE, M-DSUB, M-SILENT, M-APPROVE, M-JTCR/M-PROF/M-EVR (`20260704`), M-RATE (`20260705` + login fail-open limiter, deployed+verified); M-PWPOL (dashboard min-pw-length, user) is the only Medium left. **✅ Backup restore-drill Phase 2 PASSED (R38)** — pipeline signed off (finding: auth schema needs re-provisioning in DR). **🚀 CLIENT-01 LIVE in prod (R39)** — client ID `XX-0-NNN-CC` per-user, login-by-ID-or-email (Employee/Client toggle), read-only client portal, admin "Manage logins". Phases 0–5 complete: migrations `20260706`+`20260707` applied in Studio (prod now through `20260707`); `provision-client` (NEW → 7th Edge Fn) + `login` (`--no-verify-jwt`, `identifier`-based) deployed; frontend v=98. **Phase-5 RLS audit DONE** — found+fixed 2 client-boundary leaks (`20260707`); anon 45/45, client probe 0 FAIL. **▶ Next priority:** in-app **client smoke** (last gate before a real client) → **closeout** (template wording · leave pro-rating · bilingual Help page) → **roster swap (LAST, RSK-0, 3 confirms).** Details in [PENDING_TASKS.md](PENDING_TASKS.md) Round 39. *(Blocks below kept for history; their framing is obsolete.)*
